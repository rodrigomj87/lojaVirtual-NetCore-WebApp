...
namespace QuitandaOnline.Pages
{
    ...
    public class FinalizarPedidoModel : PageModel
    {
        ...
        public FinalizarPedidoModel(ApplicationDbContext context, IEmailSender emailSender)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            if (Request.Cookies.ContainsKey(COOKIE_NAME))
            {
                ...
                Pedido = await _context.Pedidos.Include("ItensPedido").
                    Include("ItensPedido.Produto").FirstOrDefaultAsync(p => p.CarrinhoId == cartId);

                Cliente = await _context.Clientes.FirstOrDefaultAsync(c => c.Email == User.Identity.Name);

                if ((Pedido.ClienteId > 0) && (Pedido.Endereco != null))
                {
                    ...
                    Response.Cookies.Delete(COOKIE_NAME, new CookieOptions()
                    {
                        Secure = true,
                    });
        ...
        private async Task EnviarEmailResumoPedido()
        {
            ...
            sb.Append($"<p>Recebemos seu pedido de compra número {Pedido.PedidoId.ToString("D6")}, no valor total de <b>{Pedido.ValorTotal.ToString("C")}</b>, conforme detalhamento a seguir:<p>");
            ...
            sb.Append($"<p>Att.,<br>Equipe Digital Store</p>");
            try
            {
                await _emailSender.SendEmailAsync(Cliente.Email, $"Pedido {Pedido.PedidoId.ToString("D6")}",
                    "", sb.ToString());
            }
            catch (Exception e)
            {
